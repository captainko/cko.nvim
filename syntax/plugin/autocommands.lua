...
commander.augroup("ExternalCommands", {
	{
		...
		pattern = { "png", "jpg", "gif" },
		command = function()
			vim.api.nvim_command(string.format('silent! "%s | :bw"', vim.g.open_command .. " " .. vim.fn.expand("%")))
		end,
	},
})
-- NOTE: no idea what its does. Comment it out for now
-- commander.augroup('CheckOutsideTime', {
--   {
...
commander.augroup("Utilities", {
	...
	},
	{
		...
		command = function()
			...
			if vim.bo.ft ~= "gitcommit" and pos > 0 and pos <= vim.fn.line("$") then
				pcall(vim.api.nvim_command, 'keepjumps normal g`"')
				pcall(vim.api.nvim_command, "loadview") -- load folds
			end
		end,
	},
	{
	...
	},
	{
		...
		command = function()
			...
			if utils.is_empty(vim.bo.filetype) or vim.fn.exists("b:ftdetect") == 1 then
				vim.cmd([[
unlet! b:ftdetect
filetype detect
